...
def item_detail_by_type(request, item_type, id):
    ...
    if request.method == "GET":
        ...
        if item_type == "Car":
            serializer = CarSerializer(item.car, data=request.data, partial=True)
        elif item_type == "FloorMat":
            floormat = item.floormat
            serializer = FloorMatSerializer(floormat, data=request.data, partial=True)
        elif item_type == "FlowerDecoration":
            flower_decoration = item.flowerdecoration
            serializer = FlowerDecorationSerializer(
                flower_decoration, data=request.data, partial=True
            )
        elif item_type == "Platoon":
            platoon = item.platoon
            serializer = PlatoonSerializer(platoon, data=request.data, partial=True)
        else:
            return Response(
                {"message": "Invalid item type"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def clients_count(request):
    try:
        client = MyUser.objects.filter(is_superuser=False).count()
        return Response({"booking_count": client})
    except Coordinator.DoesNotExist:
        return Response({"error": "Clinets not found"}, status=404)


@api_view(["GET"])
def venders_count(request):
    try:
        venders = Venders.objects.all().count()
        print(venders)
        return Response({"booking_count": venders})
    except Coordinator.DoesNotExist:
        return Response({"error": "Clinets not found"}, status=404)


@api_view(["GET"])
def coordinators_count(request):
    try:
        coordinators = Coordinator.objects.all().count()
        print(coordinators)
        return Response({"booking_count": coordinators})
    except Coordinator.DoesNotExist:
        return Response({"error": "Clinets not found"}, status=404)
...
def event_revenue(request):
    try:
        bookings = EventUserBook.objects.all()
        total_earnings = sum(booking.event.price for booking in bookings)
        print(total_earnings)
        return Response({"total_earnings": total_earnings})
    except EventUserBook.DoesNotExist:
        return Response({"error": "event user not found"}, status=404)
